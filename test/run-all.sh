...
TDIR=$(dirname "$0")
fail=0
for t in verify test-config test-kill test-mouse test-workspaces test-strut test-randr test-tray test-restart test-tasklist test-comp test-nested-float test-click-delivery test-focus-steal test-ghost-unmap; do
    echo "=== $t ==="
    out=$(mktemp)
    "$TDIR/$t.sh" >"$out" 2>&1
    rc=$?
